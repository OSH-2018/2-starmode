    ...
    if (strcmp(args[0], "env") == 0){
        ...
        if (!args[1]){
            ...
            for (int i=1; *(environ + i); i++){
                s = *(environ + i);
                strcat(out+strlen(out), s);
                strcat(out+strlen(out), "\n");
            }
            strcat(out+strlen(out), "\0");
            return out;
        }
        //获取单个环境变量
...
char * usePipes(char *pipeArgs[32][8192], int i){
...
char * useTwoPipes(char *pipeArgs[32][8192], int i){
    int sod = dup(STDOUT_FILENO), sid = dup(STDOUT_FILENO);
    int cd[2], fd[2];
...
int oneStruct(char *args[8192]){
    if ((strcmp(args[0], "cd") == 0 || strcmp(args[0], "pwd") == 0 ||
         strcmp(args[0], "exit") == 0 || strcmp(args[0], "env") == 0 ||
